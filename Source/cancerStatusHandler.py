...
def findPatientIDColumn(cancerStatusWS):
    for column in range(1, cancerStatusWS.max_column + 1):
        if cancerStatusWS.cell(column=column, row=1).value == "Medrio ID":
            return column
...
def validateCancerStatus(patientDict, cancerStatusWS, writeWS):
    ...
    for row in range(2, cancerStatusWS.max_row + 1):
        wsPatientIDCell = cancerStatusWS.cell(column=patientIDColumn, row=row)
        ...
        if wsPatientIDCell.value in patientDict:
            ...
            try:
                ...
                writeWS.cell(column=cancerColumn, row=patientDict[wsPatientIDCell.value].cellLocation[1] + 1,
            ...
            if patientDict[wsPatientIDCell.value].cancerStatus.lower() == wsPatientCancerStatusCell.value.lower():
                print('Cancer Status matched for patient ' +
                      str(wsPatientIDCell.value))
            else:
                writeWS.cell(column=cancerColumn, row=patientDict[wsPatientIDCell.value].cellLocation[1] + 1,
    ...
    if len(patientList) != 0:
        ...
        for patient in patientList:
            print('Patient ID: ' + str(patient) +
                  ' has not been found in the Cancer/Non-cancer workbook')
            writeWS.cell(column=cancerColumn, row=patientDict[patient].cellLocation[1] + 1,
                         value='Patient not found in Cancer/Non Cancer Medrio Report')
            errorLogList.append('Patient ID: ' + str(patient) +
                                ' has not been found in the Cancer/Non-cancer workbook')
...
def validateSiteLocations(patientDict, cancerStatusWS, writeWS):
    ...
    for row in range(2, cancerStatusWS.max_row + 1):
        patientIDCell = cancerStatusWS.cell(column=patientIDColumn, row=row)
        ...
        if patientIDCell.value in patientDict:
            ...
            if siteLocationCell.value is None:
                errorLog.append(
                    'Patient ID ' + str(patientIDCell.value) + ' does not have a site location')
                print('Patient ID ' + str(patientIDCell.value) +
                      ' does not have a site location')

    # check for left over patients
    if len(patientList) != 0:
        for patient in patientList:
            errorLog.append('Patient ID ' + str(patient) +
                            ' was not found in the Cancer/Non-cancer workbook.')
...
def getSubjectStatus(patientDict, cancerStatusWS, writeWS):
    ...
    for row in range(2, cancerStatusWS.max_row + 1):
        ...
        if patientIDCell.value in patientDict:
            ...
            writeWS.cell(column=subjectStatusColumn, row=patientDict[patientIDCell.value].cellLocation[1] + 1,
